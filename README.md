# Hi, I'm Sayantan Bhattacharya

**B.Tech CS (AI & ML) Major** at Techno Main Salt Lake, Kolkata  
Building real projects. Open to internships and freelance work.

---

##  Featured Project

### [PassME — Full-Stack Password Manager](https://github.com/PopcornKitsilano/PassME)
> Built with Python, Flask, SQLite · Argon2 hashing · Fernet encryption · OWASP Top 10 protections  
> 📺 [Watch Demo on YouTube](https://www.youtube.com/watch?v=X2SUDquMxqc)

---

## 🛠️ Tech Stack

**Languages**
`Python` `JavaScript` `C` `C++` `Java` `SQL` `Bash`

**Web & Frameworks**
`Flask` `Django` `React` `Node.js` `HTML5` `CSS3` `Bootstrap` `REST APIs`

**Cybersecurity**
`BurpSuite` `Metasploit` `Wireshark` `Hydra` `Splunk` `Graylog` `OWASP Top 10`

**Tools**
`Git` `GitHub` `Linux` `VS Code` `Arduino`

---

## 📜 Certifications

| Certificate | Issuer | Year |
|---|---|---|
| [CS50X — Introduction to Computer Science](https://certificates.cs50.io/2bd9d6d1-fde6-4b09-86c6-358a7c3d5cd3.pdf?size=letter) | Harvard University via edX | 2025 |
| [PreSecurity Certificate](https://tryhackme.com/certificate/THM-6A59K7WNUT) | TryHackMe — 100+ rooms completed | 2025 |
| [Getting Started with AI](https://www.credly.com/badges/1ce518b3-5079-46bb-8abd-2a96ce32d5e3) | IBM via Credly | 2024 |

---

## 📊 GitHub Stats

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=PopcornKitsilano&layout=compact&theme=tokyonight&hide_border=true&langs_count=6)

![Streak](https://streak-stats.demolab.com?user=PopcornKitsilano&theme=tokyonight&hide_border=true)

---

## 📫 Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/sayantan-bh/)
[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:sayantanbhattacharyauni@gmail.com)
[![YouTube](https://img.shields.io/badge/PassME_Demo-FF0000?style=for-the-badge&logo=youtube&logoColor=white)](https://www.youtube.com/watch?v=X2SUDquMxqc)

---

*3rd year AI and machine learning MAJOR · Kolkata, India · Open to remote internships and freelance projects*
